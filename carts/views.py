    ...
    try:
        ...
        if request.user.is_authenticated:
            cart_item = CartItem.objects.get(product=product, user=request.user,
                                             id=cart_item_id)# select the particular cart item that belongs to the user
        #user not logged in
        else:
...
def cart(request, total=0, quantity=0, cart_items=None):
    try:
        ...
        # cart items for NON logged in users
        else:
    ...
    try:
        ...
        # cart items for NON logged in users
            ...
            cart = Cart.objects.get(
                cart_id=_cart_id(request))  # the session_key, hence the _cart_id is persistent on the website
            cart_items = CartItem.objects.filter(cart=cart, is_active=True)
        for cart_item in cart_items:
            total += (cart_item.product.price * cart_item.quantity) #total amount of all product in the cart
            quantity += cart_item.quantity #total quantity in all the cart
